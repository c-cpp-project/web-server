c++ -Werror main.cpp ./setting/ConfigFunctions.cpp ./setting/ConfigParser.cpp ./setting/Location.cpp ./setting/Server.cpp ./ServerConfiguration.cpp WebServer.cpp \
	 Event.cpp StringUtils.cpp SocketUtils.cpp \
	 ./mock/Handler.cpp \
	 ../HttpRequest/HttpRequestFactory.cpp \
	 ../HttpRequest/HttpRequest.cpp \
	 ../HttpRequest/HttpRequestParser.cpp \
	 ../HttpRequest/RequestLine.cpp \
	 ../HttpRequest/HttpHeaders.cpp \
	 ../HttpRequest/RequestParams.cpp \
	 ../HttpRequest/RequestUtility.cpp \
	 ../HttpRequest/HttpRequestHandler.cpp \
	 ../controllers/*.cpp \
	 ../ResponseHandler/*.cpp \
	 ../HttpConfig.cpp
	#  ./mock/Handler.cpp 

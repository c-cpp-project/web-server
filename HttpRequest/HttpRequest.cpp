...
HttpRequest::HttpRequest(const HttpRequest& ref) { *this = ref; }
...
int HttpRequest::addRequestParamsToQueryString(
    const std::string& query_string) {
  if (request_params == NULL) return (FAILURE);
...
void HttpRequest::printAllHeader() const {
  ...
  // for(it = request_headers->getBegin(); it != request_headers->getEnd();
...
void HttpRequest::printAllParams() const {
  if (request_params == NULL) return;
  std::map<std::string, std::string>::iterator it;
  //   for (it = request_params->getBegin(); it != request_params->getEnd();
  //   it++)
...
void HttpRequest::setQueryString(std::string queryString) {
  if (request_line == 0) return;
  request_line->setQueryString(queryString);
}

void HttpRequest::setRepository(std::string value) { repository += value; }

std::string HttpRequest::getRepository(void) { return (repository); }
...
void DefaultController::service(HttpRequest &request, HttpResponse &response) {
  ...
  // std::cout << "[" <<
  // serverConfig->getRedirectionPath(request.getPath()).second << "]\n";
  // std::cout << "============ 2 serverConfig->getPort() ========\n";
  if (isAcceptableMethod(request.getMethod()) == false) throw "405";
  ...
  if (redirectPath != "") {
    ...
    staticPath =
        request.getPath();  // 4. 없으면 무엇을 반환하는가? -> error를 던진다.
    ...
    response.putHeader("Content-Type",
                       ResponseConfig::getContentType(request.getPath()));

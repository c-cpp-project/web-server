...
void ChildProcess::earseChildProcess(pid_t pid) {
  ...
  iter = child.find(pid);
  if (iter != child.end()) child.erase(iter->first);
}
  ...
  while (iter != child.end()) {
    ...
    } else  // 종료 되었을 경우
    {
      ...
      if (WIFEXITED(status))  // 정상 종료
        std::cout << "Child process" << child_pid << " exited with status ["
                  << WEXITSTATUS(status) << "]\n";
      else  // 비정상 종료 -> fd가 닫히면 이벤트 감지에 의해 runBeanEvent가
            // 실행된다.
        std::cout << "Child process" << child_pid << " exited with status ["
                  << WEXITSTATUS(status) << "]\n";
    }

...
TestConfiguration::TestConfiguration()
    : port(80),
      ...
      keepAliveTimeout(60) {
  locationMap.insert(
      std::make_pair("/", Location("/usr/share/nginx/html", "index.html")));
  locationMap.insert(std::make_pair(
      "/images", Location("/usr/share/nginx/images", "index.html")));
}
...
int TestConfiguration::getPort() const { return port; }

bool TestConfiguration::getSendFile() { return sendFile; }

int TestConfiguration::getKeepAliveTimeout() { return keepAliveTimeout; }
...
const Location& TestConfiguration::getErrorpageLocation(int statusCode) {
  std::map<int, Location>::const_iterator it =
      errorPageLocation.find(statusCode);

...
HttpRequest* HttpRequestFactory::create(int socket_fd,
  ...
  try {
    ...
    if (request != NULL) {  // chunked 요청인 경우
      long chunked_request_size = parseChunkedRequest(
          request, server_config, HttpRequestHandler::getBuffer(socket_fd));
      HttpRequestHandler::removePartOfBuffer(socket_fd, 0,
                                             chunked_request_size);
    } else {  // 일반 요청인 경우
      HttpRequestParser::parse(request, server_config,
                               HttpRequestHandler::getBuffer(socket_fd));
      removeRequestInBuffer(socket_fd, request);
    }
  } catch (const int&
               e)  // 특별한 예외(완전하지 않은 요청, 청크 시작 및 끝 요청) 처리
  {
  ...
  return (
      request);  // NULL인 경우, 버퍼에 파싱하기에 충분한 데이터가 없음을 뜻함
}

long HttpRequestFactory::parseChunkedRequest(HttpRequest* request,
                                             ServerConfiguration* server_config,
  ...
  int body_size_length = RequestUtility::countDigits(
      client_body_size);  // client body size의 자릿 수
  if (end_of_chunk_size == std::string::npos) {
    if (buffer.size() >=
        body_size_length + 2)  // "client body size\r\n" 까지 충분히 읽었다면
  ...
  // 청크 크기 파싱하기
  long chunk_size =
      RequestUtility::hexToDecimal(buffer.substr(0, end_of_chunk_size));
  ...
  if (buffer.size() < end_of_chunk_size + chunk_size + 4)
    throw INCOMPLETE_REQUEST;  // 버퍼에 청크 크기 만큼의 청크 데이터가 없는
                               // 경우
...
void HttpRequestFactory::SpecialExceptionHandling(const int& e, int socket_fd,
                                                  HttpRequest*& request) {
  switch (e) {
    ...
    case START_CHUNKED_REQUEST:  // 청크 전송 시작 요청
      HttpRequestHandler::addChunkedRequest(
          socket_fd, request);  // chunkeds 맵에 request 객체 추가
      removeRequestInBuffer(socket_fd, request);
      break;
    case LAST_CHUNKED_REQUEST:  // 청크 전송 끝 요청
      HttpRequestHandler::removeChunkedRequest(
          socket_fd);  // chunked 맵에서 request 객체 삭제
      HttpRequestHandler::removePartOfBuffer(socket_fd, 0,
                                             5);  // 5는 "0\r\n\r\n"의 길이
      break;
  }
...
void HttpRequestFactory::removeRequestInBuffer(int socket_fd,
                                               HttpRequest* request) {
  if (!request) return;

  // 헤더 부분까지 삭제
  size_t end_of_headers =
      HttpRequestHandler::getBuffer(socket_fd).find("\r\n\r\n") + 4;

...
void HttpRequestParser::parseRequestHeaders(HttpRequest *request,
                                            ServerConfiguration *&server_config,
                                            const std::string &buffer,
                                            long &start) {
  // 요청 헤더가 들어왔는지 확인하기
  ...
  if (end_of_headers ==
      std::string::npos)  // 요청 헤더의 끝을 식별할 수 없는 경우
  {
    if (buffer.size() >=
        server_config
            ->getClientHeaderSize())  // 최대 헤더 크기만큼 충분히 읽었다면
  ...
  while (true) {
    ...
    if (nl_pos == start) break;  // 헤더 부분이 끝난 경우 while문 탈출
...
bool HttpRequestParser::isExistBody(HttpRequest *request) {
  ...
  if (request->getHeader("Transfer-Encoding") ==
      "chunked")  // TODO : chunked 이외는 다 무시하자.. 괜찮나?
  {
  ...
  if (request->getHeader("Content-Length") == "")
    throw "411";  // Transfer-Encoding, Content-Length 둘 다 없는 경우 -> 411
                  // 응답

  return (true);
}

void HttpRequestParser::parseRequestBody(HttpRequest *request,
                                         ServerConfiguration *server_config,
                                         const std::string &buffer,
                                         long start) {
  // content_length 구하기
  ...
  if (content_length > server_config->getClientBodySize(request->getPath()))
    throw SocketCloseException413();  // content_length가 제한된 본문 크기를
                                      // 초과하는 경우
...
void HttpRequestParser::parseRequestParams(HttpRequest *request) {
  ...
  if (request->addRequestParamsToQueryString(request->getQueryString()) ==
      FAILURE)
    throw "400";

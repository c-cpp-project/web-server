...
MyController::MyController(int masking, std::string mLocation)
    : Controller(masking, mLocation) {}
...
std::string MyController::findFullPath(std::string fullpath, std::string page) {
  ...
  // file에 확장자 없을 경우: 가장 처음으로 만나는 동일한 이름의 파일에
  // 대응된다.
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (S_ISDIR(buf.st_mode) && d_name == file) {
      ...
      break;
                ...
                d_name.substr(0, d_name.find(".")) == file) ||
               file == d_name)
      break;
  }
  if (entry == NULL) throw "404";
  return (directory + "/" + d_name);
}

std::string MyController::getLocationIndex(ServerConfiguration *serverConfig,
                                           std::string path) {
...
void MyController::runCgiScript(HttpRequest &request, HttpResponse &response) {
  ...
  try {
    ...
    if (request.getPath() == mainChain)
      ...
      fullpath = std::string(root + "/" +
                             request.getPath().substr(mainChain.length() + 1));
    ...
    {
      int fd =
          open(fullpath.c_str(), O_CREAT | O_TRUNC | O_RDWR, S_IRUSR | S_IWUSR);
      if (fd < 0) throw "500";
      close(fd);
    }
    targetPath = fullpath;
  ...
  if (request.getMethod() == "GET" ||
      request.getHeader("CONTENT-TYPE") ==
          "application/x-www-form-urlencoded")  // get, post
...
void MyController::runService(HttpRequest &request, HttpResponse &response) {
  ...
  if (location != NULL && location->getAutoIndex() &&
      request.getPath() == serverConfig->findLocationUri(request.getPath()))
    response.listDirectory(location->getRoot());
  else if (redirectPath != "") {
    response.setStatusCode(
        serverConfig->getRedirectionPath(request.getPath()).first);
    ...
    if (request.getPath() ==
        serverConfig->findLocationUri(request.getPath()))  // /root/index_file
    {
    ...
    } else  // /root/file_name
    {
      ...
      mainChain = serverConfig->findLocationUri(request.getPath());
      // std::cout << request.getPath().substr(mainChain.length() + 1) << " =
      // pendingElements\n";
      staticPath = findFullPath(
          std::string(root + "/" +
                      request.getPath().substr(mainChain.length() + 1)),
          index);
    }
...
void MyController::service(HttpRequest &request, HttpResponse &response) {
  ...
  if (extension != "" && request.getPath().length() >= extension.length() &&
      request.getPath().substr(request.getPath().length() -
                               extension.length()) == extension) {
    if (request.getMethod() == "POST") allowedMethod = true;
  }

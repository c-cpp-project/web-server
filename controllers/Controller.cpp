...
void Controller::classifyEvent(HttpRequest &request, HttpResponse &response,
                               const char *cgi_python) {
  int pid;
  int pipefd1[2];
  int pipefd2[2];
  char const *argv[3];
  ...
  if (pid == 0) {
    ...
    close(pipefd2[0]);
    dup2(pipefd2[1], STDOUT_FILENO);  // 출력
    close(pipefd2[1]);
    if (request.getMethod() == "POST" &&
        request.getHeader("CONTENT-TYPE") !=
            "application/x-www-form-urlencoded") {
      close(pipefd1[1]);
      dup2(pipefd1[0], STDIN_FILENO);  // 입력
      close(pipefd1[0]);
    }
...
void Controller::writeEventRegister(int writefd[2], int readfd[2],
                                    HttpResponse &response, std::string data) {
  ...
  event->saveEvent(writefd[1], EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0,
                   new HttpHandler(writefd[1], data, serverConfig));
...
void Controller::readEventRegsiter(int readfd[2], HttpResponse &response,
  ...
  event->saveEvent(readfd[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0,
                   new HttpHandler(readfd[0], response,
                                   bodySize));  // EVFILT_READ, EVFILT_WRITE
}
...
char **Controller::envpList(HttpRequest &request) {
  ...
  for (iter = request.getHeaderBegin(); iter != request.getHeaderEnd(); iter++)
    size++;
  size += 9;  // METHOD, SERVER_PROTOCOL, PATH_INFO, REQUEST_URI, CONTENT_TYPE,
              // PATH_TRANSLATED; SERVER_PROTOCOL; SERVER_SOFTWARE,
              // GATEWAY_INTERFACE
  if (request.getQueryString() != "") size++;  // Query String
  ...
  envp[idx++] = strdup(
      ("CONTENT_TYPE=" + ResponseConfig::getContentType(request.getPath()))
          .c_str());
  envp[idx++] =
      strdup(("SERVER_PROTOCOL=" + request.getProtocolString()).c_str());
  envp[idx++] = strdup(std::string("SERVER_SOFTWARE=NGINX").c_str());
  envp[idx++] = strdup(std::string("GATEWAY_INTERFACE=CGI/1.1").c_str());
  for (iter = request.getHeaderBegin(); iter != request.getHeaderEnd();
       iter++) {
    std::string key;
    std::string value;
    std::string env;
    ...
    key = iter->first;
    if (iter->first.find("ACCEPT") != std::string::npos ||
        iter->first.find("CONTENT-TYPE") != std::string::npos)
      continue;
    if (iter->first.find("SEC-") != std::string::npos) continue;
    if (iter->first.find("CONTENT-LENGTH") == std::string::npos &&
        iter->first.find("SERVER-PROTOCOL") == std::string::npos &&
        iter->first.find("SERVER_PORT") == std::string::npos &&
        iter->first.find("SCRIPT-NAME") == std::string::npos)
...
std::pair<std::string, std::string> Controller::getFileName(
    HttpRequest &request) {
  std::string filename = "";
  ...
  if (S_ISDIR(buf.st_mode)) {
    if (request.getHeader("CONTENT-DISPOSITION") != "") {
      filename =
          request.getHeader("CONTENT-DISPOSITION")
              .substr(
                  request.getHeader("CONTENT-DISPOSITION").find("filename="));
      s = filename.find('\"');
      e = filename.find('\"', s + 1);
    ...
    if (filename.find(".") == std::string::npos) {
      type = request.getHeader("content-Type")
                 .substr(request.getHeader("content-Type").find("/") + 1);
      if (type == "") type = "txt";
...
void Controller::doGet(HttpRequest &request, HttpResponse &response) {
  ...
  if (request.getQueryString() == "") request.setQueryString(request.getBody());
...
Controller::Controller() { this->masking = 1; }

Controller::Controller(int masking) { this->masking = masking; }
